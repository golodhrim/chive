...
class BrowsePage extends CModel
{
    ...
    protected $lastResultSetQuery;

    protected $originalQueries = array();
    protected $executedQueries = array();

    protected $nonEditableSchemas = array(
        'information_schema'
        #'mysql'
    );
    ...
    public function run()
    {
        ...
        if ($this->execute) {
            ...
            if ($profiling) {
                ...
                $response->addNotification('success',
                    Yii::t('core', 'successExecuteQuery'),
                    Yii::t('core', 'foundRowsQueryTime', array(null, '{rows}'=>$total, '{time}' => $time)),
                    $sqlQuery->getQuery()
                );
            }
        }
